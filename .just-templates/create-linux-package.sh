...
pkg_dir="$1"
runtime_dir="$(dirname "$pkg_dir")/../runtime/linux"
...
# ESS Community Demo - Linux Installation

## Prerequisites

- Linux (Ubuntu 20.04+, Fedora 35+, Debian 11+, etc.)
...
if [ -f "$SCRIPT_DIR/installers-linux.tar.gz" ]; then
    tar -xzf "$SCRIPT_DIR/installers-linux.tar.gz" -C "$INSTALLERS_DIR"
    echo "✓ Linux installers extracted"
fi
