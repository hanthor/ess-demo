        ...
        for img in "${IMAGE_CACHE_DIR}/kind-images"/*.tar; do
            if [ -f "$img" ]; then
                ...
                if command -v container_load_image >/dev/null 2>&1; then
                    container_load_image "$img" "${CONTAINER_RUNTIME:-docker}"
                else
    ...
    if kind get clusters 2>/dev/null | grep -q "^${CLUSTER_NAME}$"; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            if command -v container_kind_delete >/dev/null 2>&1; then
                container_kind_delete "$CLUSTER_NAME" "${CONTAINER_RUNTIME:-docker}"
            else
...
EOF
    ...
    if command -v container_kind_create >/dev/null 2>&1; then
        container_kind_create "$CLUSTER_NAME" "${CONTAINER_RUNTIME:-docker}" --config "$KIND_CONFIG"
    else
